...
contract('WaxToken', (accounts) => {
  ...
  it('should have INITIAL_SUPPLY set to 10e15 Grains', () => {
    return WaxToken.deployed().then((instance) => {
      return instance.INITIAL_SUPPLY.call()
    })
    .then((supply) => {
      assert.equal(supply, 10e15, "10e15 wasn't the value of INITIAL_SUPPLY Grains")
    })
  })

  it('should set totalSupply to 10e15 Grains', () => {
    return WaxToken.deployed().then((instance) => {
      return instance.totalSupply.call()
    })
    .then((supply) => {
      assert.equal(supply, 10e15, "10e15 wasn't the value of totalSupply Grains")
    })
  })

  it('should put 10e15 Grains in the first account', () => {
    return WaxToken.deployed().then((instance) => {
      return instance.balanceOf.call(accounts[0])
    })
    .then((balance) => {
      assert.equal(balance.valueOf(), 10e15, '10e15 Grains were not assigned to the first account')
    })
  })
